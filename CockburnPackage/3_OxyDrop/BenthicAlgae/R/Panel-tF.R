  ...
  ifelse(max(chem,na.rm = T)>0
          ,y.axis.sequence      <- round_any(seq(from=min(chem,na.rm = T)*.95,to=max(chem,na.rm = T)*1.05
                                       ,length.out=tick.master.y)
                                   ,roundy,f=f.choice)
          ,y.axis.sequence      <- round_any(seq(from=max(chem,na.rm = T)*.95,to=min(chem,na.rm = T)*1.05
                                       ,length.out=tick.master.y)
                                   ,roundy,f=f.choice)
          );y.axis.sequence
  ...
  #axis(2,cex.axis=0.7,cex.lab=1.0,tck=T,tcl=-0.2,line=0,at=y.axis.sequence,las=1)
  ...
  mtext(2,text=expression("Sediment water flux (mmol m "^-2*"y "^-1*")"),cex=axis.label.size,line=5.0)
  par(xpd=T)
  # legend(
