# dailyfluxlabels<-function(thisfile){
  ...
  if(thisfile=="ca")        {label.bold=expression(bold("Ca"^"2+"*" (mmol m"^-2*" d"^-1*")")        );factor=1}
  if(thisfile=="caco3")     {label.bold=expression(bold("CaCO"[3]*" (mmol m"^-2*" d"^-1*")")        );factor=1}
  ...
  if(thisfile=="MPB")       {label.bold=expression(bold("MPB (mmol C m"^-2*" d"^-1*")")             );factor=1}#(1e3/1e4)}
  ...
  if(thisfile=="salinity")  {label.bold=expression(bold("Salinity (PSU m"^-2*" d"^-1*")")           );factor=1}#(1e3/1e4)}
  # label<<-label
# }
