...
time.s<-timey[seq(from=2,to=length(timey),by=subsample)]
...
#                 )
par(xpd=F)
points(time.s, mas.sum                     ,type="h",col=mass.colours[1]
        ,lwd=10,lend="butt"
)
...
points(time.s,(mas.2.sum+mas.3.sum)          ,type="h",col=mass.colours[3]
      # points(timey,(mas.2.sum)          ,type="h",col=mass.colours[3]
        ,lwd=10,lend="butt"
)
points(time.s, mas.3.sum                     ,type="h",col=mass.colours[4]
        ,lwd=10,lend="butt"
)
box(bty="o")
