...
y.axis.sequence      = round_any(seq(from=max(bw.sed.depths)
                                     #,to=min(bw.sed.depths)
                                     ,to=0
                                     ,length.out=(max(bw.sed.depths)/y.seq.num+1)
                                     )
                                     ,y.seq.num
                                     #,f=floor
                                     ,f=ceiling
...
if(timeindices[1]==1){timeindices[1]=2};timeindices
...
mtext(2,text=left.axis.text,cex=axis.label.size,line=5.0) # 1=bottom, 2=left, 3=top, 4=right
par(xpd=T)
legend(
  # inset = legend.inset
...
max.conc<-max(linematrix[,3])
max.conc<-signif(max.conc,2);max.conc
mass.conc.index<-which(files[i]==colnames(simulation.table))
simulation.table[5,mass.conc.index]<-max.conc; simulation.table
