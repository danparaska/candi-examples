...
if(time.start.index==1){time.start.index=3}; time.start.index
...
C_at_t<-as.matrix(file[1+time.start.index:time.stop.index
             ,3:thecol  ])
#C_at_t.2<-as.matrix(as.numeric(C_at_t),nrow=dim(C_at_t)[2] , ncol=dim(C_at_t)[1] )
class(C_at_t) <- "numeric"
...
for (d in 2:(depthindex-1)) {
  weighteddepths[d]<-(somedepths[d]-somedepths[d-1])/(somedepths[depthindex-2])#-somedepths[maxd-1])
};weighteddepths
...
weighted.C<-sweep(C_at_t, STATS = weighteddepths[2:length(weighteddepths)], MARGIN = 2, FUN = "*")
# weighted.C<-  C_at_t * weighteddepths[3:depthindex]
average.C<-rowSums(weighted.C, na.rm=T)
max(average.C)
...
y.axis.sequence = round_any(seq(from=min(average.C,na.rm = T),to=max(average.C,na.rm = T)
                                ,length.out=tick.master.y),roundy,f=floor);y.axis.sequence
if(y.axis.sequence[length(y.axis.sequence)]-y.axis.sequence[2]<=roundy){roundy=roundy/10}
if(y.axis.sequence[2]-y.axis.sequence[1]<=roundy){roundy=roundy/10}
y.axis.sequence = round_any(seq(from=min(average.C,na.rm = T),to=max(average.C,na.rm = T)
                                ,length.out=tick.master.y),roundy,f=floor);y.axis.sequence
...
plot(time.y, average.C
     ...
     ,col = "white"
     ,xaxs="i"
     ,ylim=c(min(average.C),max(average.C))
...
mtext(3,text="Time (number of years)",cex=axis.label.size,line=2.0, adj=0.55)
mtext(3,text=the.label,cex=axis.label.size,line=2.0, adj=-0.3)
# mtext(3,text=threetext,cex=axis.label.size,line=2,col="navy")
mtext(1,text="Time (date)",cex=axis.label.size,line=2.0, adj=0.55)
par(xpd=F)
depth.text<-as.character(avdep)
left.axis.text<-expression("Average"  ) ;left.axis.text
mtext(2,text=left.axis.text,cex=axis.label.size,line=4.9 ) # 1=bottom, 2=left, 3=top, 4=right
mtext(2,text=paste0(" over ",depth.text, " cm"),cex=axis.label.size,line=4) # 1=bottom, 2=left, 3=top, 4=right
par(xpd=T)

mean.av.C<-mean(average.C)
mean.av.C<-signif(mean.av.C,2)
