  ...
  dim(row1)
  dim(con.s.bw)
  image.plot(time.s,bw.sed.depths,con.s.bw
             ,axes=F,ylab="",xlab=""
             ,ylim=c(max(bw.sed.depths),-1*bw.depth)
             ,zlim=c(z.min,z.max)
             ,xlim=c(min(time.s),max(time.s))
             ,bigplot=c(0.20,0.85,0.12,0.70)# 1 left; 2 right; 3 bottom; 4 top
             ,smallplot=c(0.88,0.90,0.11,0.70)# 1 left; 2 right; 3 bottom; 4 top
             ,col=newglen(5000)
             ,xaxs = "i"
             ,axis.args=list(cex.axis=axis.label.size*1.0)
  ...
  box(bty="o")
  par(mgp=c(1.5,1,0))
  axis(2,cex.axis=axis.label.size,cex.lab=1.0,tck=T,tcl=-0.2,line=0
       ,at=y.axis.sequence,las=1)
  
  axis(3,tck=T,tcl=-0.3,xaxs="i",yaxs="i",line=0,col.axis="white"
       ,at=axis.sequence.raw
  ) # The ticks
  axis(3,cex.axis=axis.label.size,xaxs="i",yaxs="i",line=-0.50,lwd = 0 
       ,at=axis.sequence.raw
  ...
  # 
    ...
    mtext(3,text=the.label,cex=axis.label.size*0.9,line=2.5,adj = 0.8) # 1=bottom, 2=left, 3=top, 4=right
    mtext(3,text=unit,cex=axis.label.size*0.9,line=1.5, adj=1.2) # 1=bottom, 2=left, 3=top, 4=right
