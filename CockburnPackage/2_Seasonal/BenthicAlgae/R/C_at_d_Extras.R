...
row1<-as.numeric(as.matrix(     file[1,1:depthindex  ]    ) )           #layernum]) # Top row
the.depths<-row1[2:length(row1)]
roundy=5
bw.depth=5; bw.depths= seq(from=(-1*bw.depth), by=1, length.out=bw.depth); bw.mat=matrix(bw.depths);bw.mat
bw.sed.depths<-cbind(t(bw.mat),t(row1));bw.sed.depths
y.seq.num=5
if(max(bw.sed.depths)>45){y.seq.num=10}
if(max(bw.sed.depths)<15){y.seq.num=2.5}

y.axis.sequence      = round_any(seq(from=max(as.numeric(bw.sed.depths))
                                     #,to=min(bw.sed.depths)
                                     ,to=0
                                     ,length.out=(max(as.numeric(bw.sed.depths))/y.seq.num+1)
                                     )
                                     ,y.seq.num
                                     #,f=floor
                                     ,f=ceiling
...
y.axis.sequence[length(y.axis.sequence)+1]<- -5 ;y.axis.sequence

depthindices=NULL
for(d in 1:length(desired.depths)){
   depthindices[d]<-which.min(abs(as.numeric(the.depths) - desired.depths[d]) )
  }
timeindices=NULL
...
for(lm1 in 1:length(times_d)){
    linematrix[ ,lm1] <- t(as.matrix(file[as.numeric(timeindices[lm1])
                                ,1:(length(the.depths)+0 )]  
                                ) )
};linematrix
dim(linematrix)
linematrix<-matrix(as.numeric(linematrix)
                                ,nrow=length(the.depths)
                                ,ncol=length(times_d)
);dim(linematrix)
...
bottomwater.matrix<-matrix(nrow=1,ncol=length(times_d)
                           ,linematrix[1,]
                           ,dimnames = list(-5))
dim(bottomwater.matrix)

linematrix.2<-rbind(bottomwater.matrix,linematrix)
dim(linematrix.2)
rownames(linematrix.2)=c(-5,the.depths)
colnames(linematrix.2)=time[timeindices]
...
ifelse( abs(max(times,na.rm=T))>1
        ,roundx.CD   <- 10^floor(log10(abs(max(as.numeric(linematrix,na.rm=T)))) )/10
        ,roundx.CD   <- 10^floor(log10(abs(min(as.numeric(linematrix,na.rm=T)))) )/10
);roundx.CD
#x.axis.sequence = round_any(seq(min(linematrix),max(linematrix),length.out=tick.master.x)
#                            ,roundx.CD,f=floor);x.axis.sequence
#if(max(x.axis.sequence) - min(x.axis.sequence) < roundx.CD ){roundx.CD = roundx.CD/10};roundx.CD
#x.axis.sequence = round_any(seq(min(linematrix),max(linematrix),length.out=tick.master.x)
#                            ,roundx.CD,f=floor);x.axis.sequence
x.axis.sequence = round_any(seq(0,max(linematrix),length.out=tick.master.x)
                            ,roundx.CD,f=floor);x.axis.sequence
if(max(x.axis.sequence) - x.axis.sequence[2] < roundx.CD ){roundx.CD = roundx.CD/10};roundx.CD
x.axis.sequence = round_any(seq(0,max(linematrix),length.out=tick.master.x)
                            ,roundx.CD,f=floor);x.axis.sequence
...
plot(linematrix.2[,1], rownames(linematrix.2)
     ...
     ,col = "white"
       # ,xaxs="i" 
     ,ylim=c(max(y.axis.sequence),-5)
     # ,ylim=c(max(the.depths),min(the.depths))
     ,xlim=c(0,max(as.numeric(linematrix)))
     ,xpd=F
)
box(bty="o")
...
axis(2,cex.axis=axis.tick.size,cex.lab=1.0,tck=T,tcl=-0.2,line=0,tck=T,tcl=-0.3,col.axis="white",at=y.axis.sequence)# Ticks
axis(2,cex.axis=axis.label.size,cex.lab=1.0,tck=F,tcl=-0.2,line=0.0,lwd=0,at=y.axis.sequence,las=1)# Labels
axis(3     ,tck=T     ,tcl=-0.3     ,line = 0, at=x.axis.sequence
     ,col.axis="white"     ,cex.axis=1     ,cex=0.1     ) # The ticks
axis(3,cex.axis=axis.label.size, line = -0.5, lwd= 0, tck=F,at=x.axis.sequence) # The labels
#threetext<-c(the.label, unit); threetext
threetext<-c("Depth profile", unit); threetext
mtext(3,text=threetext[1],cex=axis.label.size,line=2.5,col="black")
mtext(3,text=threetext[2],cex=axis.label.size,line=1.5,col="black")
...
legend(
  # inset = legend.inset
       ...
       x = max(as.numeric(linematrix[,1]))*0.75, y = max(as.numeric(the.depths))*0.5
       # ,"bottomright"
       ,bty = "o",bg = "white"
